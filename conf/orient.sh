...
curl -o orientdb.tar.gz https://abcum.s3.amazonaws.com/orientdb/orientdb-community-${VERSION}.tar.gz && mkdir -p /opt/orient && tar -zxvf orientdb.tar.gz --strip-components=1 --directory /opt/orient && rm -rf orientdb.tar.gz && rm -rf /opt/orient/config && mv /opt/config /opt/orient/config
